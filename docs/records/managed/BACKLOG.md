...
- id: BL-007
  区分: 不具合
  タスク内容: plCore.getOperatePage が 'about:brank'（誤字）と比較しているため、空白ページを読み飛ばす処理が機能しない
  優先度: P2
  状態: 未着手
  担当: AIエージェント
  完了条件: 比較対象が about:blank になっている
  依存: []
  根拠: 2026-09-13の全体レビューで検出
- id: BL-006
  区分: 不具合
  タスク内容: pageChange で切替後ではなく切替前のページに bringToFront している
  優先度: P2
  状態: 未着手
  担当: AIエージェント
  完了条件: pageChange 実行後、切替後のページ（userPage）が bringToFront される
  依存: []
  根拠: 2026-09-13の全体レビューで検出
- id: BL-005
  区分: 不具合
  タスク内容: >-
    screenshot で pageIndex 指定時に wkPage.screenshot を await しておらず、保存完了前に次の操作へ進み例外も捕捉されない。
    また共有の options.screenshot へ path を書き込んでおり、連番が1000以上になると000へ戻り既存ファイルを上書きする
  優先度: P2
  状態: 未着手
  担当: AIエージェント
...
- id: BL-002
  ...
  状態: 未着手
  担当: AIエージェント
  完了条件: npm audit --omit=dev で js-yaml の指摘が0件になり、サンプルYAML3件が従来と同一の内容で読み込める
  依存: []
  根拠: >-
    2026-09-13の品質ゲート定義時に検出。当初は playwright と一括の予定だったが、2026-09-13の自律ループで
    js-yaml（同一メジャー内の更新で自動確認可能）と playwright（BL-018、実行確認は BL-019 の人手検証）に分割した
- id: BL-001
  区分: 品質ゲート
